...
def move_block(pub_cmd, loop_rate, start_xw_yw_zw, target_xw_yw_zw, vel, accel):
    ...
    rospy.sleep(1.0)
    start_dest_high = lab_invk(float(start_xw_yw_zw.x), float(start_xw_yw_zw.y), float(.15), 0)
    ...
    if True:
        ...
        rospy.sleep(1.0)
        end_dest_high = lab_invk(float(target_xw_yw_zw.x), float(target_xw_yw_zw.y), float(.15), 0)
...
def main():
	...
	sub_hokuyo = rospy.Subscriber('spur/laser/scan', LaserScan, laser_callback)
  	...
  	spawn_block(0,False)
	rospy.sleep(1)
	block_world = hokuyo2world(range_min, range_max, angle_min, angle_max, angle_increment, ranges)
	...
	#	print("rosrun project_py project_exec.py theta1 theta2 theta3 theta4 theta5 theta6 \n")
	#else:
