...
def move_block(pub_cmd, loop_rate, start_xw_yw_zw, target_xw_yw_zw, vel, accel):
    ...
    rospy.sleep(1.0)
    start_dest_high = lab_invk(float(start_xw_yw_zw.x), float(start_xw_yw_zw.y), float(.1), 0)
    ...
    if digital_in_0:
        ...
        rospy.sleep(1.0)
        end_dest_high = lab_invk(float(target_xw_yw_zw.x), float(target_xw_yw_zw.y), float(.1), 0)
